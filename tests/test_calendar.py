"""Офлайн-тесты ICS-генератора и HTTP-сервера (без сети и БД).

TODO: Marina — реализовать после ical/generator.py и ical/server.py
"""
...
def test_deadline_without_due_at_skipped():
    ...
    ics = events_to_ics([], [d])
    # дедлайн без срока не должен попадать в ICS
    assert ics.count("BEGIN:VEVENT") == 0


# ── HTTP endpoint (с mock db) ─────────────────────────────────────────────────

def test_ical_endpoint_returns_200(monkeypatch):
    pytest.importorskip("fastapi.testclient")
    from fastapi.testclient import TestClient
    from ical.server import make_app
    import db

    monkeypatch.setattr(db, "get_telegram_id_by_ical_token", lambda t: 100)
    monkeypatch.setattr(db, "get_schedule", lambda tid, since, until: [_event()])
    monkeypatch.setattr(db, "list_deadlines", lambda chat_id, **kw: [])

    client = TestClient(make_app("http://localhost"))
...
def test_ical_endpoint_404_on_bad_token(monkeypatch):
    ...
    monkeypatch.setattr(db, "get_telegram_id_by_ical_token", lambda t: None)
