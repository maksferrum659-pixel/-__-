...
def format_week(events: list[ScheduleEvent]) -> str:
    ...
    for e in events:
        ...
        if key != current_day:
            if day_lines:
                blocks.append("\n".join(day_lines))
            wd = _WEEKDAYS_RU[e.starts_at.weekday()]
            day_lines = [f"\n<b>{wd}, {e.starts_at.strftime('%d.%m')}</b>"]
            current_day = key
...
def _fmt_deadline(d: Deadline) -> str:
    ...
    when = d.due_at.strftime("%d.%m %H:%M") if d.due_at else "срок не указан"
...
def format_credits(events: list[ScheduleEvent], deadlines: list[Deadline]) -> str:
    ...
    for e in sorted(events, key=lambda e: e.starts_at):
        lines.append(f"🎓 {escape(e.discipline_name)} — {e.starts_at.strftime('%d.%m %H:%M')}"
